...
2. Optimization: A local LangGraph node uses Phi-3-mini to rewrite the prompt into a concise, professional instruction. Phi-3-mini was selected due to its low memory footprint, strong instruction-following capability, and efficient CPU-based inference, making it well-suited for local prompt optimization under constrained hardware environments.
...
3. Cloud Execution (Groq API):
   The Optimized Prompt is dispatched to the Groq endpoint.
...
| Metric              | Original (Raw) | Optimized (Phi-3 Mini) | Efficiency Gain |
|---------------------|----------------|------------------------|-----------------|
| Avg. Query Length   | 154 chars      | 68 chars               | ~55% Reduction  |
| Avg. Token Cost     | High ($$$)     | Low ($)                | Cost-Effective  |
